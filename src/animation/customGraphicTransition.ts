import {
  TRANSFORMABLE_PROPS,
  TransformProp,
} from 'zrender/lib/core/Transformable'
import {
  AnimationOption,
...
export type ElementRootTransitionProp =
  | TransformProp
  | 'shape'
  | 'extra'
  | 'style'
...
function getElementAnimationConfig(
  ...
  const config: ElementAnimateConfig =
    getAnimationConfig(animationType, parentModel, dataIndex) || {}
  ...
  if (config.duration > 0) {
    ...
    config.during = userDuring
      ? bind(duringCall, { el: el, userDuring: userDuring })
      : null
  ...
  if (hasAnimation) {
    if (isInit) {
      ...
      each(ELEMENT_ANIMATABLE_PROPS, (propName) => {
        const prop: TransitionOptionMixin = propName
          ? elOption[propName]
          : elOption
        if (prop && prop.enterFrom) {
          if (propName) {
            ;(enterFromProps as any)[propName] =
              (enterFromProps as any)[propName] || {}
          }
          extend(
            propName ? (enterFromProps as any)[propName] : enterFromProps,
            prop.enterFrom
          )
        }
      })
...
function prepareShapeOrExtraTransitionFrom(
  ...
  const attrOpt: Dictionary<unknown> & TransitionOptionMixin = (
    elOption as any
  )[mainAttr]
  if (!attrOpt) {
    return
...
function prepareStyleTransitionFrom(
  ...
  const fromElStyle = (fromEl as LooseElementProps)
    .style as LooseElementProps['style']
...
export function updateLeaveTo(el: Element, elOption: TransitionElementOption) {
  ...
  for (let i = 0; i < ELEMENT_ANIMATABLE_PROPS.length; i++) {
    ...
    if (prop && prop.leaveTo) {
      ...
      extend(
        propName ? (leaveToProps as any)[propName] : leaveToProps,
        prop.leaveTo
      )
    }
  }
...
function applyMiscProps(el: Element, elOption: TransitionElementOption) {
  // Merge by default.
  hasOwn(elOption, 'silent') && (el.silent = elOption.silent)
  hasOwn(elOption, 'ignore') && (el.ignore = elOption.ignore)
  if (el instanceof Displayable) {
    hasOwn(elOption, 'invisible') &&
      ((el as Path).invisible = elOption.invisible)
  }
  if (el instanceof Path) {
    hasOwn(elOption, 'autoBatch') &&
      ((el as Path).autoBatch = elOption.autoBatch)
  }
}

export function isTransitionAll(
