...
import { collectReferCoordSysModelInfo, getAxisMainType } from './helper'
import {
  bind,
  each,
  indexOf,
...
import { asc, linearMap } from '../../util/number'
...
interface Displayables {
  sliderGroup: Group
  handles: [Path, Path]
  handleLabels: [Text, Text]
  dataShadowSegs: Group[]
  filler: Rect
...
class SliderZoomView extends DataZoomView {
  ...
  private _clear() {
    throttleUtil.clear(this, '_dispatchZoomAction')
  ...
  private _resetLocation() {
    ...
    const positionInfo =
      this._orient === HORIZONTAL
        ? {
            ...
            right: piSize.width - coordRect.x - coordRect.width,
            top:
              piSize.height -
              DEFAULT_FILLER_SIZE -
    ...
    const layoutRect = layout.getLayoutRect(layoutParams, piSize)
  ...
  private _findCoordRect() {
    ...
    let rect: RectLike
    const coordSysInfoList = collectReferCoordSysModelInfo(
      this.dataZoomModel
    ).infoList
  ...
  private _renderDataShadow() {
    ...
    const candlestickDim =
      seriesModel.getShadowDim && seriesModel.getShadowDim()
    const otherDim: string =
      candlestickDim && data.getDimensionInfo(candlestickDim)
    ...
    if (
      ...
      data.each([otherDim], function (value: ParsedValue, index) {
        ...
        const otherCoord = isEmpty
          ? 0
          : linearMap(value as number, otherDataExtent, otherShadowExtent, true)
    ...
    function createDataShadowGroup(isSelectedArea?: boolean) {
      const model = dataZoomModel.getModel(
  ...
  private _renderHandle() {
    ...
    const handles: [graphic.Path, graphic.Path] = (displayables.handles = [
      null,
      null,
    ])
    const handleLabels: [graphic.Text, graphic.Text] =
      (displayables.handleLabels = [null, null])
    const sliderGroup = this._displayables.sliderGroup
    const size = this._size
    const dataZoomModel = this.dataZoomModel
    const api = this.api
    ...
    sliderGroup.add(
      new Rect({
        ...
        style: {
          // deprecated option
          stroke:
            dataZoomModel.get('dataBackgroundColor' as any) ||
    ...
    if (brushSelect) {
      const moveHandleHeight = parsePercent(
      ...
      moveHandle.ensureState('emphasis').style = dataZoomModel
        .getModel(['emphasis', 'moveHandleStyle'])
        .getItemStyle()

      const moveZoneExpandSize = Math.min(
        size[1] / 2,
  ...
  private _updateInterval(handleIndex: 0 | 1 | 'all', delta: number): boolean {
    ...
    sliderMove(
      ...
      minMaxSpan.minSpan != null
        ? linearMap(minMaxSpan.minSpan, percentExtent, viewExtend, true)
        : null,
      minMaxSpan.maxSpan != null
        ? linearMap(minMaxSpan.maxSpan, percentExtent, viewExtend, true)
        : null
    )
  ...
  private _updateDataInfo(nonRealtime?: boolean) {
    ...
    function setLabel(this: SliderZoomView, handleIndex: 0 | 1) {
      ...
      handleLabels[handleIndex].setStyle({
        ...
        y: textPoint[1],
        verticalAlign:
          orient === HORIZONTAL ? 'middle' : (direction as ZRTextVerticalAlign),
  ...
  private _renderBackground() {
    ...
    // Click panel, over shadow, below handles. xy轴的轨道
  ...
  private _onClickPanel(e: ZRElementEvent) {
    const size = this._size
    const localPoint = this._displayables.sliderGroup.transformCoordToLocal(
      e.offsetX,
      e.offsetY
    )
  ...
  private _updateBrushRect(mouseX: number, mouseY: number) {
    ...
    const startPoint = sliderGroup.transformCoordToLocal(
      brushStart.x,
      brushStart.y
    )
