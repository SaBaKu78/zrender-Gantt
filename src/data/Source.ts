...
import { DatasetOption } from '../component/dataset/DatasetModel'
 ...
 */
export function createSourceFromSeriesDataOption(
  data: OptionSourceData
): Source {
...
export function detectSourceFormat(
  ...
  if (isTypedArray(data)) {
    ...
    for (const key in data) {
      if (
        hasOwn(data, key) &&
        isArrayLike((data as Dictionary<unknown>)[key])
      ) {
...
function determineSourceDimensions(
  ...
  if (sourceFormat === SOURCE_FORMAT_ARRAY_ROWS) {
    ...
    if (!dimensionsDefine) {
      dimensionsDefine = objectRowsCollectDimensions(
        data as OptionSourceDataObjectRows
      )
    }
