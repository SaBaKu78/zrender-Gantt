...
export function isDimensionStacked(
  data: SeriesData,
  stackedDim: string
): boolean {
  ...
  return (
    !!stackedDim && stackedDim === data.getCalculationInfo('stackedDimension')
  )
}

export function getStackedDimension(
  data: SeriesData,
  targetDim: string
): DimensionName {
...
> {
  ...
  if (stackedDimInfo) {
    ...
    stackResultDimension = '__\0pistackresult_' + seriesModel.id
    stackedOverDimension = '__\0pistackedover_' + seriesModel.id
...
function isLegacyDimensionsInput(
  ...
  return !isSeriesDataSchema(
    (dimensionsInput as EnableDataStackDimensionsInput).schema
  )
}
