import { ImagePatternObject, PatternObject, SVGPatternObject } from 'zrender'
...
import { SVGVNode } from 'zrender/lib/svg/core'
...
export function createOrUpdatePatternFromDecal(
  ...
  function setPatternnSource(pattern: PatternObject) {
    ...
    if (isValidKey) {
      ...
      if (cache) {
        isSVG
          ? ((pattern as SVGPatternObject).svgElement = cache as SVGVNode)
    ...
    function brushDecal() {
      ...
      function brushSymbol(
        ...
        if (isSVG) {
          const symbolVNode = (zr.painter as SVGPainter).renderOneToVNode(
            symbol
          )
          if (symbolVNode) {
