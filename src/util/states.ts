...
import { DisplayableState } from 'zrender/lib/graphic/Displayable'
import {
  BlurScope,
  ColorString,
  ComponentMainType,
...
import { makeInner, queryDataIndex } from './model'
...
function hasFillOrStroke(
  fillOrStroke: string | PatternObject | GradientObject
) {
...
function getFromStateStyle(
  ...
  for (let i = 0; i < props.length; i++) {
    ...
    ;(fromState as any)[propName] =
      val == null ? defaultValue && defaultValue[propName] : val
  }
...
function createEmphasisDefaultState(
  ...
  if (el instanceof Path) {
    ...
    const fromFill = hasSelect
      ? store.selectFill || store.normalFill
      : store.normalFill
    const fromStroke = hasSelect
      ? store.selectStroke || store.normalStroke
      : store.normalStroke
  ...
  if (state) {
    ...
    if (state.z2 == null) {
      ...
      const z2EmphasisLift = (el as PIElement).z2EmphasisLift
      state.z2 =
        el.z2 + (z2EmphasisLift != null ? z2EmphasisLift : Z2_EMPHASIS_LIFT)
    }
  }
  return state
}
...
function createSelectDefaultState(
  ...
  if (state) {
    ...
    if (state.z2 == null) {
      state = extend({}, state)
      const z2SelectLift = (el as PIElement).z2SelectLift
...
function traverseUpdateState<T>(
  ...
  el.isGroup &&
    el.traverse(function (child: ExtendedElement) {
...
export function setAsHighDownDispatcher(el: Element, asDispatcher: boolean) {
  ...
  if ((el as PIElement).highDownSilentOnTouch) {
    extendedEl.__highDownSilentOnTouch = (el as PIElement).highDownSilentOnTouch
  }
...
function singleEnterEmphasis(el: PIElement) {
  // Only mark the flag.
  // States will be applied in the echarts.ts in next frame.
  doChangeHoverState(el, 'emphasis', HOVER_STATE_EMPHASIS)
}
function singleEnterBlur(el: PIElement) {
  doChangeHoverState(el, 'blur', HOVER_STATE_BLUR)
}

function singleLeaveBlur(el: PIElement) {
  if (el.hoverState === HOVER_STATE_BLUR) {
    doChangeHoverState(el, 'normal', HOVER_STATE_NORMAL)
  }
}

function singleLeaveEmphasis(el: PIElement) {
  // Only mark the flag.
  // States will be applied in the echarts.ts in next frame.
...
function singleEnterSelect(el: PIElement) {
  el.selected = true
}
function singleLeaveSelect(el: PIElement) {
  el.selected = false
}

export function enableHoverFocus(
  el: Element,
  focus: InnerFocus,
  blurScope: BlurScope
) {
  ...
  if (focus != null) {
    piData.focus = focus
    piData.blurScope = blurScope
  } else if (piData.focus) {
    piData.focus = null
  }
}
...
export function isHighDownPayload(
  payload: Payload
): payload is HighlightPayload | DownplayPayload {
  const payloadType = payload.type
  return (
...
export function blurSeries(
  ...
  const piModel = api.getModel()
  blurScope = blurScope || 'coordinateSystem'

  function leaveBlurOfIndices(
    data: SeriesData,
    dataIndices: ArrayLike<number>
  ) {
  ...
  const targetSeriesModel = piModel.getSeriesByIndex(targetSeriesIndex)
  let targetCoordSys: CoordinateSystemMaster | CoordinateSystem =
    targetSeriesModel.coordinateSystem
  ...
  piModel.eachSeries(function (seriesModel) {
    ...
    let coordSys: CoordinateSystemMaster | CoordinateSystem =
      seriesModel.coordinateSystem
    if (coordSys && (coordSys as CoordinateSystem).master) {
      coordSys = (coordSys as CoordinateSystem).master
    }
    const sameCoordSys =
      coordSys && targetCoordSys ? coordSys === targetCoordSys : sameSeries // If there is no coordinate system. use sameSeries instead.
    if (
      !(
      ...
      view.group.traverse(function (child) {
        ...
        if (
          (child as ExtendedElement).__highByOuter &&
          sameSeries &&
          focus === 'self'
        ) {
          return
      ...
      if (isArrayLike(focus)) {
        ...
        for (let d = 0; d < dataTypes.length; d++) {
          leaveBlurOfIndices(
            seriesModel.getData(dataTypes[d] as SeriesDataType),
            focus[dataTypes[d]]
          )
        }
      }
  ...
  piModel.eachComponent(function (componentType, componentModel) {
    if (componentType === 'series') {
      return
    }
    const view = api.getViewOfComponentModel(componentModel)
    if (view && view.toggleBlurSeries) {
      view.toggleBlurSeries(blurredSeries, true, piModel)
    }
  })
}
...
export function handleGlobalMouseOutForHighDown(
  ...
  const piData = getPIData(dispatcher)
  const { dispatchers } = findComponentHighDownDispatchers(
    piData.componentMainType,
    piData.componentIndex,
    piData.componentHighDownName,
    api
  )
  if (dispatchers) {
...
export function findComponentHighDownDispatchers(
  ...
  for (let i = 0; i < dispatchers.length; i++) {
    if (getPIData(dispatchers[i]).focus === 'self') {
      focusSelf = true
      break
    }
  }
