import { Dictionary, SymbolOptionMixin, ZRColor } from './types'
import * as graphic from './graphic'
import BoundingRect from 'zrender/src/core/BoundingRect'
import { calculateTextPosition, parsePercent } from 'zrender/lib/contain/text'
import { each, isArray, retrieve2 } from 'zrender/lib/core/util'

export type PISymbol = graphic.Path & {
  __isEmptyBrush?: boolean
  setColor: (color: ZRColor, innerColor?: ZRColor) => void
  getColor: () => ZRColor
}

type SymbolCtor = { new (): PISymbol }
...
const symbolShapeMakers: Dictionary<SymbolShapeMaker> = {
  ...
  triangle: function (
    ...
    w,
    h,
    shape: InstanceType<typeof Triangle>['shape']
  ) {
    shape.cx = x + w / 2
    shape.cy = y + h / 2
    shape.width = w
...
export const symbolBuildProxies: Dictionary<PISymbol> = {}
...
export function createSymbol(
  ...
  let symbolPath: PISymbol | graphic.Image
  ...
  ;(symbolPath as PISymbol).__isEmptyBrush = isEmpty

  // TODO Should deprecate setColor
  ;(symbolPath as PISymbol).setColor = symbolPathSetColor

  if (color) {
    ;(symbolPath as PISymbol).setColor(color)
  }

  return symbolPath as PISymbol
}
