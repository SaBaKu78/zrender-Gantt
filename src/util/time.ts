...
export function getDefaultFormatPrecisionOfInterval(
  ...
  if (zrUtil.isString(formatter)) {
    ...
    if (tick.level > 0) {
      for (let i = 0; i < primaryTimeUnits.length; ++i) {
        defaults[primaryTimeUnits[i]] = `{primary|${
          defaults[primaryTimeUnits[i]]
        }}`
      }
    }
...
export function getUnitValue(
  ...
  const date = zrUtil.isNumber(value) ? numberUtil.parseDate(value) : value
